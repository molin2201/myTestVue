...
public class SysDictTypeServiceImpl implements ISysDictTypeService
{
    ...
    {
        if(SystemConst.AREA_DATA.equals(dictType)){
            List<SysDictData> dictData = new ArrayList<SysDictData>();
            List<AzArea>  azAreaList=  areaService.selectAzAreaList(new AzArea());
           if(azAreaList!=null && azAreaList.size()>0){
               for(AzArea azArea:azAreaList){
                   SysDictData sysDictData=new SysDictData();
                   sysDictData.setDictCode(azArea.getId());
                   sysDictData.setDictValue(azArea.getCode());
                   sysDictData.setDictLabel(azArea.getRemark());
